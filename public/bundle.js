...
/***/ function(module, exports, __webpack_require__) {
	...
	var _table_select2 = _interopRequireDefault(_table_select);
	...
	function settingsChanged() {
	  freq = currentFrequency();
	  table = harmonicsTable();
	  _hash2.default.add({ frequency: freq });
	  if (playing) {
	    play();play();
	  }
	}
	...
	function loadHash() {
	  var options = _hash2.default.get();
	  if (options.frequency) {
	    frequencyInput.val(options.frequency);
	    freq = currentFrequency();
	  }
	  if (options.voice) {
	    _table_select2.default.select(options.voice);
	  }
	}

	playButton.on("click", play);
	frequencyInput.on("input", (0, _debounce2.default)(settingsChanged, 300));
	frequencyButtons.on("click", function (e) {
...
/***/ function(module, exports) {
	...
	var Hash = {
	  ...
	  get: function get() {
	    return window.location.hash.substring(1).split("&").reduce(function (obj, keyVal) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ComplexInputClass = function () {
	  function ComplexInputClass(element) {
	    ...
	    this.graphics = (0, _graphics2.default)(canvas);
	  ...
	  _createClass(ComplexInputClass, [{
	    ...
	    value: function _mouseMove(event) {
	      if (this.down) {
	        var pos = this.graphics.valueAt(this._eventCoords(event));
	        this.setTo.apply(this, _toConsumableArray(pos));
	      }
	    }
	  }, {
	    key: "_eventCoords",
	    value: function _eventCoords(event) {
...
/***/ function(module, exports) {
	...
	var GraphicsClass = function () {
	  function GraphicsClass(canvas) {
	    _classCallCheck(this, GraphicsClass);
	  ...
	  _createClass(GraphicsClass, [{
	    key: "render",
	    value: function render(real, imag) {
	    ...
	    key: "valueAt",
	    value: function valueAt(_ref) {
	      var _ref2 = _slicedToArray(_ref, 2),
	          x = _ref2[0],
	          y = _ref2[1];

	      var real = x / this.cx - 1,
	          imag = 1 - y / this.cy;
	      return [real, imag];
	    }
	  }, {
	    key: "_clearBackground",
	    value: function _clearBackground() {
	    ...
	    key: "_drawVec",
	    value: function _drawVec(real, imag) {
	      this._color("blue");

	      var _coordToPixel2 = this._coordToPixel(real, imag),
	          _coordToPixel3 = _slicedToArray(_coordToPixel2, 2),
	          x = _coordToPixel3[0],
	          y = _coordToPixel3[1];
	    ...
	    key: "_fillCircle",
	    value: function _fillCircle(x, y, radius) {
	      this.ctx.beginPath();
	    ...
	    key: "_coordToPixel",
	    value: function _coordToPixel(real, imag) {
	      var x = this.cx * (1 + real),
	          y = this.cy * (1 - imag);
	      return [x, y];
	    }
	  }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var serverPath = "/tables",
	    selector = "#table-select";

	function createOption(name) {
	  return (0, _jquery2.default)("<option value=\"" + name + "\">" + name + "</option>");
	}

	var TableSelect = {
	  init: function init(change, clear) {
	    this.change = change;
	    this.clear = clear;
	    this.el = (0, _jquery2.default)(selector);
	    this.el.on("change", this._tableSelected.bind(this));
	    _jquery2.default.getJSON(serverPath).done(this._setTables.bind(this));
	  },
	  select: function select(optionName) {
	    var _this = this;

	    setTimeout(function () {
	      _this.el.val(optionName).trigger("change");
	    }, 0);
	  },
	  _tableSelected: function _tableSelected(event) {
	    var _this2 = this;

	    var selected = this.el.find("option:selected").val();
	    if (selected !== "") {
	      _jquery2.default.getJSON(serverPath + ("/" + selected)).done(function (table) {
	        return _this2.change(table, selected);
	      });
	    } else {
	      this.clear();
	    }
	  },
	  _setTables: function _setTables(_ref) {
	    var _this3 = this;

	    var tables = _ref.tables;

	    this.tables = [""].concat(tables.sort());
	    this.tables.forEach(function (name) {
	      _this3.el.append(createOption(name));
	    });
	  }
	};
